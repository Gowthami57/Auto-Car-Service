...
$date1=date("Y-m-d");
$dis= mysqli_query($conn, "SELECT * FROM discount where `endDate` < '$date1' ")or 
die("Failed to query time slots". mysqli_error());
 $count=mysqli_num_rows($dis);
 if($count !=0 ){
 
  $up= mysqli_query($conn, "UPDATE discount SET startDate='', endDate='',
   discount='' where `endDate` < '$date1'" )or die("Failed to query time slots". mysqli_error());
 }
 ?>
  <!DOCTYPE html>
<html>
...
<div class="container" >
<a class="navbar-brand text-white ml-2" href="#"><span class="fa fa-car text-danger"
 style="margin-right: 7%" aria-hidden="true"></span><strong class="ml-2">AUTO CAR SERVICE</strong></a>
...
<div  id='dis'   style='width:29%;background-color:#F7941E;'> ";
                                 ...
                                 $c=$row['centerName'];
                              $conn=mysqli_connect("localhost","root","","car_service")or 
                              die("Can't Connect to Database");
                                 $discount=mysqli_query($conn, "SELECT * FROM discount where `centerName` ='$c' ")
                                 or die("Failed to query discount". mysqli_error($conn));
                                       $row1=mysqli_fetch_array($discount);
                                           
                          if($row1['discount'] != ""){
                        
                                            echo "<h6 class='card-title' style='inline-block;text-align:center;'> 
                                             ". $row1['discount'] ."% Off</h6>";
                                          }
...
$conn=mysqli_connect("localhost","root","","car_service")or die("Can't Connect to Database");
        $discount=mysqli_query($conn, "SELECT * FROM discount where `centerName` ='$c' ")or
         die("Failed to query discount". mysqli_error($conn));
              $row1=mysqli_fetch_array($discount);
