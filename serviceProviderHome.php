...
$c=$_SESSION['centerN'];
//$c='DEV AUTOMOTIVES';
$displaySlot=mysqli_query($conn, "SELECT * FROM slot_booking where centerName='$c' ")or 
                      ...
                      <p class="mt-3 mx-2"><b>Car Type : </b> '.$display["car_type"].'</p> 
                       ...
                       echo '<form method="post">  
                          ...
                          <button name="delete" class="btn btn-danger"style="margin-bottom: 5%">Delete</button>
                           <button name="done" class="btn btn-danger"style="margin-bottom: 5%">Done</button> 
                           
                           <input type="hidden" value="'.$display["id"].'" name="customerID">

                          
                           <div class="container fixed-bottom " style="left:93%;margin:0% 0% 2% 0%;font-size:24px;">

                           <button class="btn bg-dark text-light " name="chat" style="border:1px;border-radius:70%;">
                           <i class="fa fa-wechat fa-2x" ></i></button>
...
if(isset($_POST['done'])){
 ...
 //redirect to make a bill
  ...
  $res=mysqli_fetch_assoc($que);
  $phone=$res['phone'];
    $customDetails=mysqli_query($conn, "SELECT * FROM car_users where phone= $phone")or
     die("Failed to query time slots". mysqli_error());
  $customer=mysqli_fetch_assoc($customDetails);
...
echo "<script>location.href='chatboxCenter.php'</script>";
