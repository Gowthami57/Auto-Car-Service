...
$date1=date("Y-m-d");
mysqli_query($conn, "DELETE FROM slot_booking where `date` < '$date1' ")or die("Failed to query time slots". mysqli_error());
...
if($res['status']=='Accepted'){
  echo "<script>alert('request already accepted');</script>";
}
else{
...
if(mail($to, $sub, $message, $header)){
	echo "<h4> mail to user has sent successfully to user.</h4>";
  // echo $to;
}
...
if(isset($_POST['deny'])){
 $id=$_POST['customerID'];
  ...
  echo "<script>alert('request already denied');</script>";
}
else{
