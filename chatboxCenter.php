...
include("links.php");
$users=mysqli_query($connect, "SELECT * FROM service_center WHERE email='". $_SESSION['user']."' ")or 
...
<div class="container-fluid " style="background-image: url('images/chatbox.png');"  >
	...
	<div class="row">
			<a href="serviceProviderHome.php" style="color:black; font-size:1.5em; border:5px solid white;
			 background-color: white; border-radius: 6px; margin: 2% 5% 1% 92%; position:fixed;">Back</a>
		<div class="col-md-4" style="color: white; margin-top: 2%; font-family: serif;">
			...
			<h4>Send message to :</h4>
							<?php
				$u= $_SESSION['user'];
     $msgs=mysqli_query($connect, "SELECT username,phone from car_users where phone IN 
	 (select distinct rec.sender from (select * from chatbox where receiver='$u')as rec) ")
...
<div class="modal-dialog">
		<div class="model-content" style="border: 1px solid; border-color: lightgray; border-radius: 5%; width: 80%; background-color: white; height: 90.4%">
         ...
         $userName=mysqli_query($connect, "SELECT * FROM car_users WHERE phone='".$_GET["toUser"]."'")
			...
			<?php
               if(isset($_GET["toUser"])){
    	$chats=mysqli_query($connect, "SELECT * FROM chatbox where(sender='". $_SESSION['user']."' AND receiver= '".$_GET["toUser"]."')
		 OR (sender='". $_GET["toUser"]."' AND receiver= '".$_SESSION['user']."') ORDER BY id ASC")
                or die("Failed to query database". mysqli_error());
               ...
               while($chat=mysqli_fetch_assoc($chats)){
                        ...
                        if($chat["sender"] == $_SESSION['user'])
                        	echo "<div style='text-align:right;'>
                             <p style='background-color:#009999; color:white; word-wrap:break-word;display:inline-block;
							  padding:5px;border-radius:0px 8px 0px 8px;max-width:70%; '>".$chat["message"]."
                             ...
                             <p style='background-color:grey;color:white; word-wrap:break-word;display:inline-block; 
							 padding:5px;border-radius:0px 8px 0px 8px;max-width:70%; '>".$chat["message"]."
			...
			<div class="modal-footer">
				...
				<input type="submit" id="send" name="send" class="btn btn-primary" style="height: 6%; position: absolute;
				 top: 82%; left: 68%" value="Send">
				</form>
			</div>
